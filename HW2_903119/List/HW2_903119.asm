...
_0x0:
	.DB  0x54,0x65,0x6D,0x70,0x72,0x61,0x74,0x75
	.DB  0x72,0x65,0x3D,0x25,0x66,0x0
_0x2000003:
...
; 0000 00AA while (1)
_0x6:
; 0000 00AB       {int  adc_result;
; 0000 00AC       char lcd_buffer[25];
; 0000 00AD       float  temprature;
; 0000 00AE       float final_temp;
; 0000 00AF       lcd_clear();
	SBIW R28,35
;	adc_result -> Y+33
;	lcd_buffer -> Y+8
...
;	final_temp -> Y+0
	RCALL _lcd_clear
; 0000 00B0       adc_result=read_adc(0);
	LDI  R26,LOW(0)
	RCALL _read_adc
	STD  Y+33,R30
	STD  Y+33+1,R31
; 0000 00B1 //      sprintf(lcd_buffer,"ADC=%d",adc_result);
; 0000 00B2 //      lcd_gotoxy(0,0);
; 0000 00B3 //      lcd_puts(lcd_buffer);
; 0000 00B4       temprature=(adc_result/1023.0);
	CALL SUBOPT_0x0
	MOVW R26,R30
	MOVW R24,R22
	__GETD1N 0x447FC000
	CALL __DIVF21
	CALL SUBOPT_0x1
; 0000 00B5       final_temp=temprature*100.0;
	CALL SUBOPT_0x2
	__GETD1N 0x42C80000
	CALL __MULF12
	CALL __PUTD1S0
; 0000 00B6       lcd_gotoxy(0,1);
	...
	LDI  R26,LOW(1)
	RCALL _lcd_gotoxy
; 0000 00B7       sprintf(lcd_buffer,"Temprature=%f",final_temp);
	MOVW R30,R28
	ADIW R30,8
	ST   -Y,R31
	ST   -Y,R30
